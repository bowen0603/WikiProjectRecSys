...
class RecommendExperienced():
    ...
    def check_candidate_pool(self):

        self.dict_article_projects = self.read_article_all_projects()

        print("#### Working on {} newcomers... ####".format(len(self.set_valid_newcomers)))
        list_project_candidate_edits, list_project_candidate_overlaps = self.check_history(self.set_valid_newcomers, True)
        print("### Printint out analysis on newcomer candidates.. ###")
        self.formate_project_candidates(list_project_candidate_edits, list_project_candidate_overlaps,
                                        "data/candidate_analysis_newcomers_simplified.csv")


        print("#### Working on {} experienced editors... ####".format(len(self.set_valid_exp_editors)))
        list_project_candidate_edits, list_project_candidate_overlaps = self.check_history(self.set_valid_exp_editors, False)
        print("### Printint out analysis on experience editor candidates.. ###")
        self.formate_project_candidates(list_project_candidate_edits, list_project_candidate_overlaps,
                                        "data/candidate_analysis_expeditors_simplified.csv")


    def formate_project_candidates(self, list_project_candidate_edits, list_project_candidate_overlaps, output):
        dict_projects_candidates = {}
        fout = open(output, "w")
        for project in list_project_candidate_edits:
            list_candidate_edits = list_project_candidate_edits[project]

            list_project_candidate_edits[project] = sorted(list_candidate_edits, reverse=True)
            dict_projects_candidates[project] = len(list_candidate_edits)

        import operator
        dict_projects_candidates = sorted(dict_projects_candidates.items(), key=operator.itemgetter(1), reverse=True)
        for project, candidate_cnt in dict_projects_candidates:
            if project in list_project_candidate_overlaps:
                print("{},{},{}".format(project, candidate_cnt, list_project_candidate_overlaps[project]), file=fout)
            else:
                print("{},{},0".format(project, candidate_cnt), file=fout)
            # print(list_project_candidate_edits[project], file=fout)



    def check_history(self, editor_list, is_newcomers):
        editor_cnt = 0

        list_project_candidate_edits = {}
        list_project_candidate_overlaps = {}
        ...
        for editor_text in editor_list:
            ...
            try:
                ...
                query = self.url_usercontb + "uclimit=" + str(
                    self.const_max_requests) + "&ucprop=title|timestamp|parsedcomment|flags|ids&ucuser=" + editor_text
                response = requests.get(query).json()
            ...
            invalid_candidate_project = []
            for project in projects_within_30days:
                if not is_newcomers:
                    # experienced editors have a different requirement
                    if projects_within_30days[project] >= 2:
                        if project in list_project_candidate_edits:
                            list_project_candidate_edits[project].append(projects_within_30days[project])
                        else:
                            list_project_candidate_edits[project] = [projects_within_30days[project]]
                    else:
                        invalid_candidate_project.append(project)
                else:
                    if project in list_project_candidate_edits:
                        list_project_candidate_edits[project].append(projects_within_30days[project])
                    else:
            ...
            for project in invalid_candidate_project:
                del projects_within_30days[project]

            if len(set(projects_within_30days)) != 1:
                for project in projects_within_30days:
                    if project in list_project_candidate_overlaps:
                        list_project_candidate_overlaps[project] += 1
                    else:
                        list_project_candidate_overlaps[project] = 1

        return list_project_candidate_edits, list_project_candidate_overlaps
    ...
    @staticmethod
    def read_article_all_projects():
        print("### Reading all projects of the articles from file... ###")
        import json, os
        filename = os.getcwd() + "/data/pre-processing/articles_all_projects.json"
        article_projects = {}
        if os.path.isfile(filename):
            for line in open(filename, 'r'):
                try:
    ...
    def read_sample_projects():
        ...
        filename = "data/pre-processing/all_valid_wikiprojects.csv"
        list_projects = []
    ...
    def execute(self):
        ...
        self.check_candidate_pool()

        # TODO: test new bonds-based

        # recommend uu cf based editors
        # self.uucf.recommend_editors()


    def single_editor_edits(self, editor_text):
        editor_text = "Bobo.03"
        ...
        try:
            ...
            for usercontrib in response['query']['usercontribs']:
                ...
                # check the revision contents
                if ns == 3:
                    revision_query = "https://en.wikipedia.org/w/api.php?action=query&format=json" \
                                     "&prop=revisions&rvprop=content&rvstartid="+revid+"&rvend="+revid+"&rvuser="+editor_text
                    response = requests.get(query).json()

                    for revision in response['query']['revisions']:
                        print(revision)
